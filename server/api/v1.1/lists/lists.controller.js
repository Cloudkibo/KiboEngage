    ...
    .then(pages => {
      ...
      pages.forEach((page, i) => {
        ...
        facebookApiCaller('v2.11', `me/custom_labels?access_token=${page.accessToken}`, 'post', {'name': tag})
          .then(label => {
            if (label.body.error) {
              sendOpAlert(label.body.error, 'lists controller in kiboengage from createTag', page._id, page.userId._id, page.companyId)
          ...
          utility.callApi('pages/query', 'post', {_id: tag.pageId})
            .then(pages => {
              ...
              facebookApiCaller('v2.11', `me/custom_labels?access_token=${page.accessToken}`, 'post', {'name': label})
                .then(label => {
                  if (label.body.error) {
                    sendOpAlert(label.body.error, 'lists controller in kiboengage from editList', page._id, page.userId._id, page.companyId)
    ...
    utility.callApi('pages/query', 'post', {_id: tag.pageId})
      .then(pages => {
        ...
        facebookApiCaller('v2.11', `${tag.labelFbId}?access_token=${page.accessToken}`, 'delete', {})
          .then(label => {
            if (label.body.error) {
              sendOpAlert(label.body.error, 'lists controller in kiboengage From deleteListFromFacebook', page._id, page.userId._id, page.companyId)
          ...
          facebookApiCaller('v2.11', `${tagPayload.labelFbId}/label?access_token=${subscriber.pageId.accessToken}`, 'post', {'user': subscriber.senderId})
            .then(assignedLabel => {
              if (assignedLabel.body.error) {
                sendOpAlert(assignedLabel.body.error, 'lists controller in kiboengage from assignTagToSubscribers1', '', req.user._id, req.user.companyId)
          ...
          utility.callApi('tags/query', 'post', {tag, pageId: subscriber.pageId._id, companyId: req.user.companyId})
            .then(tagPayload => {
              ...
              facebookApiCaller('v2.11', `${tagPayload.labelFbId}/label?access_token=${subscriber.pageId.accessToken}`, 'post', {'user': subscriber.senderId})
                .then(assignedLabel => {
                  if (assignedLabel.body.error) {
                    sendOpAlert(assignedLabel.body.error, 'lists controller in kiboengage from assignTagToSubscribers2', '', req.user._id, req.user.companyId)
                    sendErrorResponse(res, 500, `Failed to associate tag to subscriber ${assignedLabel.body.error}`)
                  }
                  let subscriberTagsPayload = {
                    tagId: tagPayload._id,

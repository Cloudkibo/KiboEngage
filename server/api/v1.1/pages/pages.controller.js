                  ...
                  needle('get', `https://graph.facebook.com/v2.6/me?access_token=${page.accessToken}`)
                    .then(response => {
                      if (response.body.error) {
                        sendOpAlert(response.body.error, 'pages controller in kiboengage', page._id, page.userId._id, page.companyId)
                                  ...
                                  needle('post', `https://graph.facebook.com/v2.11/me/broadcast_reach_estimations?access_token=${page.accessToken}`)
                                    .then(reachEstimation => {
                                      if (reachEstimation.body.error) {
                                        sendOpAlert(reachEstimation.body.error, 'pages controller in kiboengage', page._id, page.userId._id, page.companyId)
                                      }
                                                ...
                                                needle.post(`https://graph.facebook.com/v3.2/me/subscribed_apps?access_token=${page.accessToken}`, bodyToSend, (error, response) => {
                                                  ...
                                                  needle.request('post', requesturl, valueForMenu,
                                                    {json: true}, function (err, resp) {
                                                      ...
                                                      if (resp.body.error) {
                                                        sendOpAlert(resp.body.error, 'pages controller in kiboengage', page._id, page.userId._id, page.companyId)
                                                      }
                                                    })
...
function createTag (user, page, tag, req) {
  ...
  needle('post', `https://graph.facebook.com/v2.11/me/custom_labels?access_token=${page.accessToken}`, {'name': tag})
    .then(label => {
      if (label.body.error) {
        sendOpAlert(label.body.error, 'pages controller in kiboengage', page._id, page.userId._id, page.companyId)
      }
      if (label.body.id) {
        let tagData = {
          tag: tag,

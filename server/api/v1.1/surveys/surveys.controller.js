                            ...
                            if (err) {
                              ...
                              if (webhook && webhook.optIn.SURVEY_CREATED) {
                                var data = {
                                  ...
                                  payload: JSON.stringify({userId: req.user._id, companyId: companyUser.companyId._id, title: req.body.survey.title, description: req.body.survey.description, questions: req.body.questions})
                                }
        ...
        .then(planUsage => {
          ...
          callApi.callApi('featureUsage/companyQuery', 'post', {companyId: companyUser.companyId._id}, req.headers.authorization)
            .then(companyUsage => {
              ...
              //   })
              // }
              callApi.callApi(`pages/query`, 'post', {companyId: companyUser.companyId._id, connected: true}, req.headers.authorization)
                .then(userPage => {
                  userPage = userPage[0]
                                                    ...
                                                    utility.applyTagFilterIfNecessary(req, subscribers, (taggedSubscribers) => {
                                                      ...
                                                      utility.applySurveyFilterIfNecessary(req, subscribers, (repliedSubscribers) => {
                                                        ...
                                                        for (let j = 0; j < subscribers.length && !abort; j++) {
                                                          callApi.callApi(`featureUsage/updateCompany`, 'put', {query: {companyId: companyUser.companyId._id}, newPayload: { $inc: { surveys: 1 } }, options: {}}, req.headers.authorization)
                                                            .then(updated => {
                                                              callApi.callApi('featureUsage/companyQuery', 'post', {companyId: companyUser.companyId._id}, req.headers.authorization)
                                                                .then(companyUsage => {
                                                                  companyUsage = companyUsage[0]
        ...
        .then(planUsage => {
          ...
          callApi.callApi('featureUsage/companyQuery', 'post', {companyId: companyUser.companyId._id}, req.headers.authorization)
            ...
            */
                          ...
                          surveyQuestionsDataLayer.findSurveyWithId(survey._id)
                            .then(questions => {
                              surveyDataLayer.findOneSurvey(survey._id)
                                .then(survey => {
                                  if (questions.length > 0) {
                                    let first_question = questions[0]
                                    // create buttons
